...
static int on_cmd_market_update(nw_ses * ses,rpc_pkg * pkg,json_t * params){
    ...
    size_t market_num = json_array_size(params);
    if(market_num <= 0)
        return -__LINE__;

    uint32_t same_count = 0;
    uint32_t same_num[10] = {};
    ...
    settings.market_num += (market_num - same_count);

    int index = number;
    struct market * realloc_market = realloc(settings.markets, sizeof(struct market) * settings.market_num);
    for (size_t i = 0; i < market_num; ++i) {
        ...
        ERR_RET_LN(read_cfg_str(row, "name", &realloc_market[index].name, NULL));
        ERR_RET_LN(read_cfg_int(row, "fee_prec", &realloc_market[index].fee_prec, false, 4));
        ERR_RET_LN(read_cfg_mpd(row, "min_amount", &realloc_market[index].min_amount, "0.01"));

        json_t *stock = json_object_get(row, "stock");
        if (!stock || !json_is_object(stock))
            return -__LINE__;
        ERR_RET_LN(read_cfg_str(stock, "name", &realloc_market[index].stock, NULL));
        ERR_RET_LN(read_cfg_int(stock, "prec", &realloc_market[index].stock_prec, true, 0));

        json_t *money = json_object_get(row, "money");
        if (!money || !json_is_object(money))
            return -__LINE__;
        ERR_RET_LN(read_cfg_str(money, "name", &realloc_market[index].money, NULL));
        ERR_RET_LN(read_cfg_int(money, "prec", &realloc_market[index].money_prec, true, 0));
        ++index;
    }
    settings.markets = realloc_market;

    add_market_trade(number);
...
static int on_cmd_asset_update(nw_ses * ses,rpc_pkg * pkg,json_t * params)
{
    ...
    for (size_t i = 0; i < asset_num; ++i){
        ...
        for(size_t j = 0; j < settings.asset_num; ++j){
            if(strcmp(tmp_name, settings.assets[j].name) == 0){
                same_num[i] = 1;
                ++same_count;
            }
        }
    ...
    if (asset_num == same_count) {
        return reply_error(ses, pkg, 10, "repeat update");
    }
    
    size_t number = settings.asset_num;
    settings.asset_num += (asset_num - same_count);

    struct asset *realloc_asset = realloc(settings.assets, sizeof(struct asset) * settings.asset_num);
    ...
    for (size_t i = 0; i < asset_num; ++i) {
        ...
        if (!json_is_object(row))
            return -__LINE__;
        ERR_RET_LN(read_cfg_str(row, "name", &realloc_asset[index].name, NULL));
        ERR_RET_LN(read_cfg_int(row, "prec_save", &realloc_asset[index].prec_save, true, 0));
        ERR_RET_LN(read_cfg_int(row, "prec_show", &realloc_asset[index].prec_show, false, realloc_asset[i].prec_save));
        ++index;
    }

    settings.assets = realloc_asset;

    add_asset_balance(number);
